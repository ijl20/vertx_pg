...
public class VertxPg3 extends AbstractVerticle {
    ...
    private boolean load_data(Future<Object> fut)
    {
        JsonObject sql_client_config = new JsonObject()
              .put("url", config().getString("vertx_pg.db.url"))
              .put("user", config().getString("vertx_pg.db.user"))
              .put("password", config().getString("vertx_pg.db.password"))
        ...
        jdbc_client.getConnection(res -> {
            if (res.failed()) {
                ...
                String sql_query = "SELECT * FROM csn_sensor";
